...
def add_scratch_to_lid(info_dict, scratch_new_segments,dataset_name=None):
    ...
    for sc in range(info_dict['scratch']['count']):
        ...
        if dataset_name == 'lid': #TODO check dataset
            ...
            temp = info_dict['scratch'][str(sc) + '_aug']
...
def generate_scratch_segments(number_of_generated_images, input_normal_segment, input_scratch_segment,
    ...
    while count_img<number_of_generated_images:
        ...
        for i in ['lid_normal', 'lid_scratch']:
            ...
            info_dict[i] = load_image(info_dict_ns=info_dict[i], image_size=image_size,dataset_name=dataset_name)
        ...
        info_dict = add_scratch_to_lid(info_dict, scratch_new_segments,dataset_name=dataset_name)
        ...
        if export_before_after or count_img==0:
    ...
    for name in name_list:
        ...
        for item in gen.getdata():
            if item[0] < 30 and item[1] < 30 and item[2] < 30 and item[3] == 255:
                ...
                newData.append(item)
        gen.putdata(newData)
        combined = Image.alpha_composite(lid, gen.filter(ImageFilter.BoxBlur(0.1)))

        cv2.imwrite(os.path.join(output_scratch_lid_combined, name), np.array(combined))