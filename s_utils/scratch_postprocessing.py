...
def combine_LID(output_scratch_combined, output_scratch_segment_npy, output_normal_random_image, image_size, output_scratch_lid_combined,id_lid=1):
    ...
    for name in name_list:
        ...
        for i in range(image_size[1]):
            for j in range(image_size[0]):
                if npy[i][j] > id_lid and gen[i, j, 0] != 0:
                    FIN[i][j] = gen[i][j]
